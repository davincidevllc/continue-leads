  ...
  const loadStateData = useCallback(async (stateCode: string) => {
    ...
    try {
      const [countyRes, cityRes] = await Promise.all([
        ...
        fetch(`/api/geo/cities?state=${stateCode}&limit=25&sort=population`).then(r => r.json()),
      ]);
  ...
  const renderTargeting = () => (
    <div className="card">
      ...
      <div className="card-body">
        ...
        {/* Top Cities Preview — matches generate-pages ranking */}
        {allDerivedCities.length > 0 && (
          <div>
            <div className="d-flex justify-content-between align-items-center mb-2">
              <label className="form-label fw-semibold mb-0">
                Top {allDerivedCities.length} Cities by Population
              </label>
              <input
                type="text"
            ...
            <div className="small text-muted mb-2">
              These are the cities that will get pages generated, ranked by population (25 per state cap).
            </div>
                  ...
                  {filteredCities.slice(0, 100).map((city, idx) => (
                    <tr key={`${city.state_code}-${city.id}`}>
                      ...
                      <td className="text-end">
                        {city.population ? city.population.toLocaleString() : <span className="text-warning">—</span>}
                      </td>
