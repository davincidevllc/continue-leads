...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tables = ['site_pages','generation_jobs','generation_job_items','qa_runs','qa_findings'];
    const counts: Record<string, number> = {};
    for (const t of tables) {
      try { const r = await pool.query(`SELECT COUNT(*)::int AS c FROM ${t}`); counts[t] = r.rows[0].c; }
      catch { counts[t] = -1; }
    }
